...
try {
	...
	$longpost = $app->getChannel($channel_id, [
	...
	if (isset($longpost['raw']['st.longpo.post'][0])) {
		...
		// global post
		if (!empty($longpost['raw']['st.longpo.post'][0]['global_post_id'])) {
			$global_post = $app->getPost($longpost['raw']['st.longpo.post'][0]['global_post_id']);
		}

		$single_page = true;
		$page_title = htmlentities($longpost['raw']['st.longpo.post'][0]['title'], ENT_QUOTES) . ' &ndash; Long posts';

		// currently assumes first message raw item is longpost body
		$page_description = substr($longpost['recent_message']['raw']['st.longpo.content'][0]['body'], 0, 256) . '…';
		$title = $longpost['raw']['st.longpo.post'][0]['title'];
		$body = $longpost['recent_message']['raw']['st.longpo.content'][0]['body'];
		...
		if (!empty($longpost['raw']['st.longpo.post'][0]['category'])) {
			echo 'Filed Under: <a href="/category/'.$longpost['raw']['st.longpo.post'][0]['category'].'">"'.$longpost['raw']['st.longpo.post'][0]['category'].'"</a><br>
			';
		}
		echo '<a href="' . URL . '@' . $longpost['user']['username'] . '/' . get_slug($title) .'">Share Link</a></p>';
